...
def _load():
    path = Path(__file__).parent / "assistant.py"
    if not path.exists():
...
def test_safe_path_allows_paths_inside_workdir(tmp_path):
    assistant = _load()
    resolved = assistant.safe_path(tmp_path, "notes/todo.txt")
...
def test_safe_path_rejects_traversal(tmp_path):
    ...
    with pytest.raises(ValueError):
        assistant.safe_path(tmp_path, "../escape.txt")


def test_write_happens_when_approved(tmp_path):
    assistant = _load()
    client = MagicMock()
    client.messages.create.side_effect = _write_then_done("hello.txt", "hi there")

    result = assistant.run(client, "create hello.txt", workdir=tmp_path, approve=lambda n, i: True)

    assert (tmp_path / "hello.txt").read_text() == "hi there"
    assert "Done." in result


def test_write_is_blocked_when_denied(tmp_path):
    assistant = _load()
    client = MagicMock()
    client.messages.create.side_effect = _write_then_done("hello.txt", "hi there")

    result = assistant.run(client, "create hello.txt", workdir=tmp_path, approve=lambda n, i: False)

    assert not (tmp_path / "hello.txt").exists()
    assert "Done." in result  # loop still completes after the denied write


def test_approve_is_called_with_tool_name_and_input(tmp_path):
    assistant = _load()
    client = MagicMock()
    client.messages.create.side_effect = _write_then_done("a.txt", "data")
    approve = MagicMock(return_value=True)

    assistant.run(client, "write a.txt", workdir=tmp_path, approve=approve)

    approve.assert_called_once_with("write_file", {"path": "a.txt", "content": "data"})


def test_adaptive_thinking_is_enabled(tmp_path):
    assistant = _load()
    client = MagicMock()
    client.messages.create.side_effect = [_text_message("nothing to do")]

    assistant.run(client, "hi", workdir=tmp_path)
